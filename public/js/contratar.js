        ...
        empleados.forEach(empleado => {
            ...
            card.innerHTML = `
                ...
                <div class="px-6 pt-4 pb-2">
                    <button class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded mr-2 custom-hire-btn" onclick="handleHire('${empleado.id_empleado}')">Contratar</button>
                    <button class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded custom-discard-btn" onclick="handleDiscard('${empleado.id_empleado}')">Descartar</button>
                </div>
            `;
...
async function handleHire(empleadoId) {
    try {
        const response = await fetch('/aprobar-contratacion-empleado', {
            method: 'PUT',
            headers: {
        ...
        alert(result.message || 'Empleado contratado exitosamente');
        window.location.href = `../html/forms/asignarEmpleo.html?id_empleado=${empleadoId}`; // Redirige al formulario de asignar puesto con el ID del empleado
    } catch (error) {
        console.error('Error hiring employee:', error);
        alert('Error al contratar el empleado. Por favor, inténtelo de nuevo más tarde.');
    }
}
